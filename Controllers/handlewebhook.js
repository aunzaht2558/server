        ...
        if(size == 0){
            ...
            if ( alert.order_action == 'sell' ){
               ...
               let position_data = await binance.futuresUserTrades( alert.symbol ), markets = Object.keys( position_data );
				...
				console.info(  await binance.futuresBuy( alert.symbol, quantity, get_tp,{type: 'TAKE_PROFIT', stopPrice: get_tp}));
				...
				console.info(  await binance.futuresBuy( alert.symbol, quantity, get_sl,{type: 'STOP', stopPrice: get_sl}));               
            ...
            if ( alert.order_action == 'buy') {
               ...
               let position_data = await binance.futuresUserTrades( alert.symbol ), markets = Object.keys( position_data );
				...
				console.info(  await binance.futuresSell( alert.symbol, quantity, get_tp,{type: 'TAKE_PROFIT', stopPrice: get_tp}));
				
				var get_sl0 = Number((avgPrice * (stop_loss_price / leverage)) / 100).toFixed(pricePrecision[alert.symbol ]);
				var get_sl = Number(parseFloat(avgPrice)-parseFloat(get_sl0)).toFixed(pricePrecision[alert.symbol ]);
				
				console.info(  await binance.futuresSell( alert.symbol, quantity, get_sl,{type: 'STOP', stopPrice: get_sl}));
            }
